...
export function getFeedbackRuntime() {
  const agent = detectAgentClient();
  return { client: runtimeClient, agent, agentSource: runtimeAgentClient || process.env.LLAMA_AGENT_CLIENT ? "reported" : agent === "unknown" ? "unknown" : "detected" };
}
...
function inferCommand(method, endpoint) {
  ...
  if (/^\/api\/occam\/deals\/[^/]+\/artifacts\/[^/]+$/.test(pathname)) return "deal.read";
  ...
  if (/^\/api\/deal-memory\/[^/]+\/story$/.test(pathname)) {
    return verb === "GET" ? "memory.read" : "memory.write";
  }
...
function shouldSkipTelemetry(endpoint) {
  ...
  return pathname === "/api/feedback" || pathname.startsWith("/api/feedback/") || pathname === "/api/agent/client-events" || pathname === "/api/agent/eval-feedback";
}
    ...
    const issues = Array.isArray(data?.issues)
      ? data.issues.map((issue) => {
          const path = Array.isArray(issue?.path) && issue.path.length
            ...
            : "$";
          const detail = issue?.message || issue?.code || "Invalid value";
          return `- ${path}: ${detail}`;
        })
      : [];
    const error = new Error([message, ...issues].join("\n"));
...
export function formatErrorForDisplay(error) {
  ...
  if (/^Error(?:\[[^\]]+\])?:/u.test(message)) return message;
  const code = typeof error?.code === "string" ? `[${error.code}]` : "";
  return `Error${code}: ${message}`;
}

export function structuredBootstrapManifest(manifest) {
  if (!manifest || typeof manifest !== "object" || Array.isArray(manifest)) {
    return manifest;
  }
  const { briefing: _renderedDuplicate, ...structured } = manifest;
  return structured;
}
