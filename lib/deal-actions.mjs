 ...
 * quote them verbatim from their own input, plus the agent's own restatement in
 * value.summary, so the type alone stays a reliable filter and every record
 * reads the same way. Core enforces the same rule; this check only fails faster.
 */
...
export function validateHumanSubjectiveView(information, origin) {
  ...
  const summary = stringField(information.value, "summary");
  if (!summary) {
    throw new Error("human_subjective_view.* requires value.summary: the agent's restatement of the user's words");
  }
  if (normalizeWhitespace(summary) === normalizeWhitespace(rawText)) {
    throw new Error("human_subjective_view.* value.summary must restate value.rawText, not repeat it");
  }
  return information;
}
...
/**
 * A Deal's history only grows, so an unbounded read of a long-lived Deal is not
 * merely expensive — it is unreadable, and history nobody can load is history
 * that may as well be missing. `limit` takes the newest rows of each expansion
 * and `before` walks backwards through Deal Events by `event_seq`, so the whole
 * record stays reachable a window at a time.
 */
export function buildDealReadPath(dealId, detail = "overview", options = {}) {
  const usage =
    "Usage: llama deal read <dealId> [--detail overview|memory|files|conversation|history|all] [--limit N] [--before <event_seq>]";
  if (!dealId || dealId.startsWith("--")) throw new Error(usage);
  const expansion = {
    overview: "",
    memory: "information",
    files: "artifacts",
  ...
  // Assembled by hand rather than with URLSearchParams: the expand list has
  // always travelled with literal commas, and percent-encoding them here would
  // change the wire format for every existing caller to no purpose.
  const query = [];
  if (expansion) query.push(`expand=${expansion}`);
  ...
  if (options.before != null) {
    ...
    if (!/^\d+$/.test(before)) throw new Error("--before must be an event_seq");
    query.push(`before=${before}`);
  }
  return `/api/occam/deals/${encodeURIComponent(dealId)}${query.length ? `?${query.join("&")}` : ""}`;
}
