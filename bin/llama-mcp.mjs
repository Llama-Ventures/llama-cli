...
server.registerTool(
  "agent_bootstrap",
  {
    description: "Mandatory before Llama work: fetch the authenticated runtime contract, complete private Live Deal Page field vocabulary, and visible Llama OS skills.",
...
server.registerPrompt(
  "agent_briefing",
  {
    description: "Mandatory before Llama work: fetch the authenticated current contract and complete private Live Deal Page field vocabulary; bundled text is only an offline fallback.",
  },
  async () => {
    const headers = await getAuthHeaders();
    let text;
