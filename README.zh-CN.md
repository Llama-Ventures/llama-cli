...
暴露 **20 个 typed tools**——基本镜像 CLI 最常用的命令——外加一个
通用的 `llama_api` 逃生通道（参考 GitHub 官方 MCP server 的设计），
覆盖任何还没有 typed wrapper 的端点。
...
- **服务端 schema 漂移：** 当 API 多了一个端点，下一个 minor 版本会带
  typed wrapper。在那之前，永远可以用 MCP 的 `llama_api` 逃生通道。
